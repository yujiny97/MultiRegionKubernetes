...
export function deployToEKSspec (scope: cdk.Construct, region: string, cluster: eks.Cluster, apprepo: ecr.IRepository, roleToAssume: iam.Role) :PipelineProject {
    ...
    const deployBuildSpec = new codebuild.PipelineProject(scope, `deploy-to-eks-coronaweb-${region}`, {
        environment: {
            buildImage: codebuild.LinuxBuildImage.fromAsset(scope, `custom-image-for-eks-coronaweb-${region}`, {
